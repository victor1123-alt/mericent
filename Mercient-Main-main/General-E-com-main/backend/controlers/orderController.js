(function () {
	...
	/**
	 * List all orders (public access)
	 */
	const getOrders = async (req, res) => {
		try {
			console.log('Fetching all orders (public access)');
			const orders = await OrderDb.find({}).sort({ createdAt: -1 }).populate('items.productId', 'productName');
	...
	const createOrderPublic = async (req, res) => {
		...
		try {
			...
			if (!shipping || typeof shipping.fee !== 'number' || !shipping.state) {
				return res.status(400).json({ success: false, message: 'Shipping information with fee and state is required' });
			}
