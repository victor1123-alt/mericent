          ...
          <div className="md:w-1/2 w-full">
            ...
            <div className="mt-6 flex gap-2">
              <button
                ...
                className={`w-full py-2 rounded ${isOutOfStock ? 'bg-gray-400 text-gray-600 cursor-not-allowed' : 'bg-primary text-white hover:bg-blue-600'}`}
              >
                {isOutOfStock ? 'Out of Stock' : 'Add to Cart'}
              </button>
              <button
                onClick={() => { addToCart({ ...product, size: selectedSize, color: selectedColor }); onClose(); navigate('/checkout'); }}
                disabled={isOutOfStock}
                className={`w-full py-2 rounded ${isOutOfStock ? 'bg-gray-400 text-gray-600 cursor-not-allowed' : 'bg-gray-100 dark:bg-gray-700'}`}
              >
