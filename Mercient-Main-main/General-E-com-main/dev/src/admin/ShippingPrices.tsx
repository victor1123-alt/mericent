...
const ShippingPrices: React.FC = () => {
  ...
  return (
    <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-md mt-4">
      <div className="flex justify-start md:justify-between items-start md:items-center mb-4 lg:flex-row flex-col gap-2">
      ...
      <div className="md:hidden space-y-3">
        {options.map((option) => (
          <div key={option._id} className="border border-gray-300 dark:border-gray-600 rounded-lg p-3 bg-white dark:bg-gray-800">
            <p><span className="font-semibold">Name:</span> {option.name}</p>
            <p><span className="font-semibold">State:</span> {option.state}</p>
            <p><span className="font-semibold">Base Price:</span> ₦{option.basePrice}</p>
            <p><span className="font-semibold">Per Item:</span> ₦{option.pricePerItem}</p>
            <p><span className="font-semibold">Max Items:</span> {option.maxItemsForBase}</p>
            <p><span className="font-semibold">Discount:</span> {option.discountActive ? `${option.discountPercentage}%` : 'None'}</p>
            <div className="mt-2 flex gap-2">
              <button
                onClick={() => handleEdit(option)}
                disabled={!isAdminAuthenticated}
                className="flex-1 bg-blue-500 text-white px-2 py-1 rounded text-sm hover:opacity-90 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                Edit
              </button>
              <button
                ...
                disabled={!isAdminAuthenticated}
                className="flex-1 bg-red-500 text-white px-2 py-1 rounded text-sm hover:opacity-90 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                Delete
              </button>
            </div>
          ...
          <tbody>
            {options.map((option) => (
              <tr key={option._id} className="hover:bg-gray-50 dark:hover:bg-gray-600">
                ...
                <td className="border border-gray-300 dark:border-gray-600 p-2">{option.discountActive ? `${option.discountPercentage}%` : 'None'}</td>
