...
export async function fetchShippingOptions(): Promise<{ state: string; price: number }[]> {
  try {
    const res = await fetch(`${API_BASE_URL}/api/shipping-prices`);
    ...
    data.forEach((option: any) => {
      if (!stateMap.has(option.state)) {
        stateMap.set(option.state, {
          state: option.state,
          price: option.pricePerItem, // Use price per item
    ...
    // Return empty array if no shipping options are available
    return [];
  }
}
...
export async function calculateShippingFee(state: string, itemCount: number): Promise<{
  ...
  try {
    const res = await fetch(`${API_BASE_URL}/api/calculate-shipping`, {
      method: "POST",
